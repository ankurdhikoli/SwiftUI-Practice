...
struct SegmentedPicker: View {
    private static let ActiveSegmentColor: Color = Color(.yellow)
    ...
    private var activeSegmentView: AnyView {
        ...
        return
            RoundedRectangle(cornerRadius: SegmentedPicker.SegmentCornerRadius)
                ...
                .frame(width: self.segmentSize.width, height: 3)
                .offset(x: self.computeActiveSegmentHorizontalOffset(), y: 30)
...
struct SegmentedPicker_Preview: PreviewProvider {
    static var previews: some View {
        SegmentedPicker(items: ["A", "B", "C"], selection: .constant(1))
    }
